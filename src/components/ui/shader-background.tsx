...
const ShaderBackground: React.FC<ShaderBackgroundProps> = ({ className = '' }) => {
  ...
  `;
    ...
    // Fragment shader - Paleta médica cian/turquesa
    const fsSource = `
    ...
    // Paleta médica - Cian/Turquesa
    const vec4 lineColor = vec4(0.0, 0.85, 0.95, 1.0);
    ...
    void main() {
      ...
      // Colores médicos - Azul profundo a Cian oscuro
      vec4 bgColor1 = vec4(0.02, 0.04, 0.10, 1.0);
      vec4 bgColor2 = vec4(0.03, 0.12, 0.20, 1.0);
  ...
  `;
    ...
    useEffect(() => {
        ...
        const resizeCanvas = () => {
            const dpr = Math.min(window.devicePixelRatio, 1.5); // Limitar para rendimiento
